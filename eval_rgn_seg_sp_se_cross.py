...
import argparse
import logging
from lib import evaluation_rgn_seg_sp_se_cross as evaluation
...
def main():
    ...
    if opt.dataset == 'coco':
        weights_bases = [
            'runs/coco_butd_rgn_seg_sp_se_bert',
    ...
    for base in weights_bases:
        ...
        if opt.dataset == 'coco':
            ...
            if opt.save_results:  # Save the final results for computing ensemble results
                save_path = os.path.join('save_results', 'Region_trainedcoco_valf30k_crossdataset_results_01_{}.npy'.format(opt.dataset))
            else:
                save_path = None
    #         print(opt.save_results)
            # Evaluate Flickr30K
            print('Cross-data Evaluate on Flickr30K')

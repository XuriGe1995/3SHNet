...
from lib.datasets import image_caption_segmentation_sp_se_new as image_caption
from lib.model_rgn_seg_sp_se import VSegModel
...
#     Sp_attns = None
    ...
    for i, data_i in enumerate(data_loader):
        ...
        if not backbone:
            images, image_lengths, segmentations, cat_onehots, Unet_segs, captions, lengths, ids = data_i
        else:
            images, segmentations, cat_onehots, Unet_segs, captions, lengths, ids = data_i
        model.logger = val_logger

        # compute the embeddings
        if not backbone:
            img_emb, cap_emb = model.forward_emb(images, segmentations, cat_onehots, Unet_segs, captions, lengths, image_lengths=image_lengths)
#             img_emb, cap_emb, sm_attns, sp_attns = model.forward_emb(images, segmentations, cat_onehots, Unet_segs, captions, lengths, image_lengths=image_lengths)
        else:
            img_emb, cap_emb = model.forward_emb(images, segmentations, cat_onehots, Unet_segs, captions, lengths)
        ...
        if img_embs is None:
            ...
            Sm_attns = np.zeros((len(data_loader.dataset), 36))
            Sp_attns = np.zeros((len(data_loader.dataset), 36, 4096))
...
#         Sm_attns[ids] = sm_attns.data.cpu().numpy().copy()
#         Sp_attns[ids] = sp_attns.data.cpu().numpy().copy()
...
#                 start = time.time()
                ...
                logger.info("calculate similarity time: {}".format(end - start))
#                 wf = open('COCO1000_our_i2t_sims.txt', 'w')
#                 for i in range(sims.shape[0]):
#                     wf.write(np.array2string(np.argsort(sims[i])[::-1][:10]))
#                     wf.write('\n')
#                 wf.close()
#                 simT = sims.T
#                 wf = open('COCO1000_our_t2i_sims.txt', 'w')
#                 for i in range(simT.shape[0]):
#                     wf.write(np.array2string(np.argsort(simT[i])[::-1][:10]))
#                     wf.write('\n')
#                 wf.close()
#                 import pdb
#                 pdb.set_trace()

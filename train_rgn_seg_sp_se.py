"""3SHNet Training script"""
#########
##### We referred to the framework implementations of VSE∞ --> https://vse-infty.github.io/ to build up our codebase. Thanks!!
#########
import os
import time
import numpy as np
...
from lib.evaluation_rgn_seg_sp_se import i2t, t2i, AverageMeter, LogCollector, encode_data, compute_sim
...
def train(opt, train_loader, model, epoch, val_loader):
    ...
    for i, train_data in enumerate(train_loader):
        ...
        if opt.precomp_enc_type == 'basic':
            images, img_lengths, segmentations, cat_onehots, Unet_segs, captions, lengths, _ = train_data
            model.train_emb(images, segmentations, cat_onehots, Unet_segs, captions, lengths, image_lengths=img_lengths)
        else:
            images, captions, lengths, _ = train_data
            if epoch == opt.embedding_warmup_epochs:
                warmup_alpha = float(i) / num_loader_iter
                model.train_emb(images, segmentations, cat_onehots, Unet_segs, captions, lengths, warmup_alpha=warmup_alpha)
            else:
                model.train_emb(images, segmentations, cat_onehots, Unet_segs, captions, lengths)
